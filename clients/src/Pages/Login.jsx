...
import './CSS/Login.css'
import Navbar from '../Components/Navbar/Navbar';
import { login, fetchCompleteUserProfile } from '../services/authService';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        try {
          const userProfile = await fetchCompleteUserProfile();
          console.log('User profile fetched:', userProfile);
          
          if (userProfile) {
            console.log('Profile loaded successfully, redirecting...');
          } else {
            console.warn('Could not fetch user profile, but continuing...');
          }
        } catch (profileError) {
        ...
        }, 1500); // Increased delay to allow profile fetch
      } else {
        setError(result.message || 'Login failed. Please try again.');
        console.error('Login failed:', result);
      }
